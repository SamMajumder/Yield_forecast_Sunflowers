...
Weather <- read.csv(here("Processed_Datasets","Climate_data.csv"))
...
### Viweing representation per year per state ###

Data %>% group_by(State,Year) %>% summarise(n()) %>% View()
...
## there are 46 unique values for Years i.e., 46 years 
...
train_KS <- KS %>% filter(Year <= 2004)

test_KS <- KS %>% filter(Year >= 2005) 
