...
## RF
#Rf_combined_model <- RF_combined[["Rf"]] 

## saving the trained model for further use ##

#saveRDS(Rf_combined_model,"Rf_combined_model.rds")
...
train_reduced_SD <- train_SD %>% select(Yield,Rfe_Imp_best_subset$Features)

test_reduced_SD <- test_SD %>% select(Yield,Rfe_Imp_best_subset$Features)
...
set.seed(1234)
indices <- CreateSpacetimeFolds(train_KS,spacevar = "SpaceVar", k=11)
...
Rfe_Imp_best_subset <- read_csv(here::here("Processed_Datasets",
